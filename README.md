...
<p align="center">
  <img src="docs/screenshots/home.png" alt="요즘 어때 홈 화면 — 지금 상황을 적으면 맞는 정책을 찾아준다" width="420">
</p>
...
- **말로 검색** — 키워드가 딱 맞지 않아도, 자기 말로 상황을 적으면 '의미'가 통하는 정책을 찾아줍니다. 예를 들어 _"공허하고 무기력해요"_ 라고만 적어도 마음건강 관련 정책을 찾아줍니다.
...
- **자격 요건 살펴보기** — 나이·지역 같은 내 조건이 정책의 자격 요건에 해당하는지 짚어줍니다. 다만 '된다/안 된다'로 단정하지 않고, 최종 확인은 시행기관 공고 기준이라는 안내를 늘 함께 답니다.
- **없는 내용을 지어내지 않음** — 자격 판정은 AI가 아니라 정해진 규칙(조건 계산)으로 하고, 정책 내용은 원문 그대로 보여줍니다. AI가 원문에 없는 말을 만들어내지 못하게 막았습니다.
- **키 없이도 동작** — API 키가 없거나 외부 서비스에 문제가 생겨도 검색이 끊기지 않도록 만들었습니다.
...
flowchart TD
    ...
    B -->|아니오| D["뜻이 통하는 정책 찾기<br/>의미로 검색 + 키워드"]
    D --> E["내 조건(나이·지역 등)이<br/>자격 요건에 맞는지 규칙으로 확인"]
    E --> F["결과 카드<br/>혜택 요약 · 신청 가능 여부<br/>'추정' 안내 · 원문 링크"]
```

- **찾기** — 사용자의 문장과 각 정책을 '의미 좌표(숫자 벡터)'로 바꿔, 뜻이 가까운 정책을 찾습니다. 같은 상황도 사람마다 다르게 말하기 때문에, 단어가 겹치지 않아도 의미로 찾습니다. 정확히 겹치는 단어를 찾는 키워드 검색도 함께 써서 놓치는 걸 줄입니다.
- **자격 확인** — 나이·소득·지역 같은 조건을 정해진 규칙으로 하나씩 따져, 자격 요건에 해당하는지 살펴봅니다. AI의 추측이 아니라 정해진 계산이라, 결과가 늘 일정하고 근거를 되짚어 확인할 수 있습니다.
- **보여주기** — 결과 카드에 혜택(무엇을 받는지) 요약, 지금 신청할 수 있는지 여부, 내 조건 중 맞는 부분과 확인이 필요한 부분, 그리고 '추정' 안내와 원문 링크를 함께 담습니다.

> 사람마다 같은 상황을 다르게 말합니다. 그래서 '의미'로 찾아 표현이 달라도 맞는 정책을 놓치지 않고, 자격은 정해진 규칙으로 따져 결과를 믿을 수 있게 했습니다.
...
- **위험한 순간이 먼저입니다.** 위험한 마음 신호가 보이면 정책 대신 상담 연결을 먼저 보여주고, 마음이 좀 진정되면 언제든 다시 검색으로 돌아올 수 있게 안내합니다. 글을 쓰는 중에는 말을 끊지 않고 안내만 곁들이고, 다 적고 나면 상담 화면으로 넘어갑니다.
- **자격을 함부로 단정하지 않습니다.** '된다/안 된다' 대신 '확인이 필요하다'고 안내합니다. 마지막 판단은 늘 시행기관의 공고 기준입니다.
- **없는 정보를 지어내지 않습니다.** 확인되지 않은 전화번호나 기관 이름은 보여주지 않고, 정책 내용은 원문 그대로 전합니다.
- **어디서 온 정보인지 밝힙니다.** 모든 결과에 '추정'이라는 안내, 원문으로 가는 링크, 데이터를 언제 받아왔는지를 함께 보여줍니다.
- **끊기지 않게 합니다.** 위험 신호 감지는 인터넷이나 서버 상태와 상관없이 항상 작동합니다.

배포하기 전에 약 920개의 자동 테스트로 이 원칙들이 깨지지 않는지 매번 확인하고, GitHub Actions에서 빌드 검사도 함께 돌려 문제가 있으면 배포되지 않게 막습니다.
...
**환경 변수는 선택 사항입니다.** `.env.example`을 `.env`로 복사한 뒤 키를 채우면 실제 데이터와 AI가 켜지고, **키가 없어도 샘플 데이터·키워드 검색으로 앱과 테스트가 온전히 동작**합니다.
