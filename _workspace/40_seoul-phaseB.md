...
**활성화 게이트**: (1) ✅ M-1 교차출처 중복/상충 억제 — **완료**(`suppressCrossSourceDuplicates`: 서울이 정본과 정규화 제목 완전일치+정본 서울/전국이면 서울 억제. 연도변형 유지=신선도 보존. 순증 무영향. 테스트 3케이스). (2) ⬜ 청년몽땅 이용약관·공공누리 확인(운영자) — **남은 유일 게이트**. 충족 후 `SEOUL_INGEST: 'on'`.
