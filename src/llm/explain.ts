...
/**
 * 결과 설명(왜 맞는지) — 정책 record 주입 그라운딩 + 후처리 환각검증.
 *
 * 안전 불변식(엄수):
...
function buildExplainPrompt(record: GroundingRecord): string {
  ...
  return [
    '아래 정책 정보만 사용해 "왜 이 정책이 관련 있어 보이는지" 쉬운 말로 1~2문장 설명하라.',
    '규칙(엄수):',
    '- 아래 정보에 없는 사실·숫자·금액·링크·다른 정책명을 절대 만들지 마라.',
    '- 자격 여부를 단정하지 마라("자격이 됩니다", "확실히", "무조건" 금지). 추정 톤.',
    '- 신청 자격은 별도 엔진이 판정하므로, 설명은 관련성만 다룬다.',
    '',
    '[정책 정보]',
    ...lines,
  ].join('\n');
